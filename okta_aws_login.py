...
def get_arns_from_assertion(assertion):
    ...
    if not arn_text:
        raise Exception('Unable to parse arn_text from assertion')
...
def get_sid_from_file(sid_cache_file_path):
    ...
    if os.path.isfile(sid_cache_file_path):
        with open(sid_cache_file_path) as sid_file:
            sid = sid_file.read()
            if len(sid) == 25:
                return sid


def get_sts_token(role_arn, principal_arn, saml_assertion):
...
def okta_mfa_login(password_login_response, app):
    ...
    codename = ''
    for inputtag in soup.find_all('input'):
        if inputtag.get('name') in ['passcode', 'code']:
            codename = inputtag.get('name')
    if not codename:
        raise Exception('codename not found')
...
def send_sms_passcode(password_login_response):
    ...
    for inputtag in soup.find_all('input'):
        if inputtag.get('name') == '_xsrfToken':
            xsrfToken = inputtag.get('value')
    if not xsrfToken:
        raise Exception('no xsrfToken found')
    ...
    session.post(send_sms_url,
                 headers=headers_dict,
                 cookies=cookie_dict,
                 verify=True)


def update_config_file(okta_aws_login_config_file_path):
    ...
    if os.path.isfile(okta_aws_login_config_file_path):
        config.read(okta_aws_login_config_file_path)
    ...
    while not idp_entry_url_valid:
        ...
        if url_parse_results.scheme == "https" and "okta.com" in idp_entry_url:
            idp_entry_url_valid = True
        else:
            print("idp_entry_url must be HTTPS URL for okta.com domain")
    if not idp_entry_url:
        raise Exception('idp_entry_url not found')
...
def write_sid_file(sid_file, sid):
    ...
    _sid_cache_file = os.open(sid_file, os.O_WRONLY | os.O_CREAT, mode=0o600)
    os.write(_sid_cache_file, sid.encode())
    os.close(_sid_cache_file)
...
def main():
    ...
    if args.verbose:
        ...
        cred_details = (
            "Credentials for the profile {} have been set. "
            "They will expire in {} minutes."
        ).format(profile_name, valid_minutes)
        print(cred_details)
